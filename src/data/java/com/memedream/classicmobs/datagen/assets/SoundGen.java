...
public class SoundGen extends SoundDefinitionsProvider {
    ...
    public SoundGen(PackOutput output) {
        super(output, ClassicMobs.MOD_ID);
    }

    @Override
    public void registerSounds() {
        this.generateNewSoundWithSubtitle(ModSounds.DODO_IDLE, "mob/dodo/idle", 3, "Dodo squawks");
        this.generateNewSoundWithSubtitle(ModSounds.DODO_DEATH, "mob/dodo/death", 1, "Dodo dies");
        this.generateNewSoundWithSubtitle(ModSounds.DODO_HURT, "mob/dodo/hurt", 1, "Dodo hurts");
    ...
    public void generateNewSound(DeferredHolder<SoundEvent, SoundEvent> event, String baseSoundDirectory, int numberOfSounds, @Nullable String subtitle, float volume, float pitch) {
        ...
        for (int i = 1; i <= numberOfSounds; i++) {
            definition.with(SoundDefinition.Sound.sound(ClassicMobs.prefix(baseSoundDirectory + (numberOfSounds > 1 ? i : "")), SoundDefinition.SoundType.SOUND).volume(volume).pitch(pitch));
        }
    ...
    public void makeNewStepjSound(DeferredHolder<SoundEvent, SoundEvent> event, String baseSoundDirectory, int numberOfSounds) {
        SoundDefinition definition = SoundDefinition.definition();
        for (int i = 1; i <= numberOfSounds; i++) {
            definition.with(SoundDefinition.Sound.sound(ClassicMobs.prefix(baseSoundDirectory + (numberOfSounds > 1 ? i : "")), SoundDefinition.SoundType.SOUND));
        }
    ...
    public void makeMusicDisc(DeferredHolder<SoundEvent, SoundEvent> event, String discName) {
        this.add(event, SoundDefinition.definition()
            .with(SoundDefinition.Sound.sound(ClassicMobs.prefix("music/" + discName), SoundDefinition.SoundType.SOUND)
                .stream()));
    }
    ...
    private void createSubtitleAndLangEntry(DeferredHolder<SoundEvent, SoundEvent> event, SoundDefinition definition, String subtitle) {
        ...
        String subtitleKey = "subtitles.classicmobs." + splitSoundName[0] + "." + splitSoundName[2];
        definition.subtitle(subtitleKey);
        LangGen.SUBTITLE_GENERATOR.put(subtitleKey, subtitle);
    }
}
